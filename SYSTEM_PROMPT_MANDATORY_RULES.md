...
## 🟢 核心规范 (Critical Rules)
**🚫 零容忍违规清单 (Zero Tolerance Checklist)**：
*   **严禁使用 `this`**：在 `<script setup>` 中使用 `this` 是死罪，会直接导致程序崩溃。
*   **严禁私自“美化”**：**禁止**添加任何用户没要求的文字标题、按钮、装饰线、装饰背景。
*   **严禁默认背景**：除非用户要求背景，否则 CSS 里**禁止出现** `background`、`backdrop-filter`、`border`。
*   **响应式定义**：优先使用 **流式 (Fluid)** 填充（100% 宽高）。禁止在频谱组件中默认使用等比缩放。
*   **物理模拟禁令**：**严禁写任何平滑/插值逻辑！** 宿主已处理。
